...
### ⭐ 2026-08-22 外部 SEO 稽核後，等周周決定的項目（明天彙報要帶）

外部產出的 SEO 報告已逐條實測比對過。**已修好上線的 6 項**（commit `3c8c6f5`）：
全站補 `<html lang>`（36 個手寫頁）、首頁改成一頁一個 H1、文章內文粉框小標從 `<p>` 改成真正的
`<h2>`（改在產生器，46 篇三語一次生效，外觀不變）、translate 三語補
robots/canonical/hreflang/WebApplication 結構化資料、sitemap 188 個網址全部補上 lastmod
（日期取自 git 最後 commit 日）、首頁 og 與 title 同步。

**報告誤判、不要再改的 5 項**（下一個 AI 若又提，直接引用這裡）：
1. 文章頁 JSON-LD 早就有 datePublished／dateModified／author／image。
2. `properties.html` 早就有 CollectionPage → ItemList → 42 筆 Accommodation 結構化資料。
3. 已有自訂 `404.html`。
4. `logo-mark.png` 的 `alt=""` 是**刻意**的：旁邊就有「周周・日本房仲」文字，裝飾圖補 alt 會被唸兩次。
5. `property.html` 的 canonical 指向 `properties.html` 是**刻意**的：那頁掛 `noindex,follow`，
   是要靠 `?id=` 才有內容的殼頁，不是 bug。

**等周周決定（尚未動手）：**
- **首頁靜態化**：報告列為最高優先，但 46 篇文章各自都有靜態 SEO 頁，首頁只有文章卡片清單是 JS 畫的。
  動這個等於改架構＝高風險，**要通關密語**。目前建議先不做。
- **單一物件個別索引**：技術可行（每個物件產靜態頁），但物件會下架、會留死頁；建議只挑長期在售的做。
- **4 個商業關鍵字落地頁**（日本買房／東京買房／日本投資房／日本房仲推薦）：周周先前傾向「補強首頁而非新開頁」，待確認。
- **圖片優化**：WebP、srcset、width/height、檔名英文化。要重新命名大量圖檔並改所有引用。
- **簡中版 noindex**：報告的選項之一，**不建議**（丟掉實際流量換理論好處）。
- **Email 名單收集**：已有 `feedback.html`，是否再加訂閱表單待定。
- **hreflang 補完**：除 translate 外還有 14 組頁面（about、tool-*、videos 等共 42 頁）缺 hreflang。
  純加標籤、零風險，周周點頭就補。
- **日文版工具頁**：六個試算工具頁的日文版尚未寫（需重寫而非直譯）。
- **git 歷史裡的供應商姓名**：清除需要改寫歷史＝要通關密語，尚未取得。


0. **物件專區：投資照片畫質偏低、日文版未做**。(a) 10 筆投資店舖已補上型錄裁切的「外觀＋平面圖」（見第 2 節），但**畫質僅堪用**；要更清晰只能換賣家的乾淨原始照，拿到後同名覆蓋 `prop-<id>-N.jpg` 即可。(b) `properties.html`/`-cn.html` 已有（含右上 🌐 繁⇄簡切換器），但**沒有 `properties-ja.html`**（周周 2026-07-10 指示日文版先不做；其他頁都有日文版，這頁還缺）；要做需比照工具頁手寫日文外殼＋讓 `properties.js` 卡片文案支援日文（目前資料是繁中）。
...
3. **文章留言功能：程式碼已上線，Firebase 主控台設定尚未全部完成**。前端 `comments.js`（Google 登入留言、觀看免登入、周周可刪任何留言、禁貼連結／謾罵／詐騙字眼、30 秒冷卻）已隨每篇文章頁上線；**剩下要在 Firebase 主控台做的**：(a) App Check 貼上 reCAPTCHA v3 **密鑰（secret key）** 後按「儲存」；(b) Firestore 規則貼上 repo 根目錄的 `firestore.rules` 並「發布」；(c) Authentication 啟用 Google 登入、把 `chouchouinjapan.com` 與 `janny00143.github.io` 加進「已授權網域」；(d) 上線實測留言成功後，**最後**才開 App Check「強制執行」。詳見第 7 節。
4. **Google Search Console 收錄**：已給周周分批網址清單，需要持續追蹤「Coverage」報表，若有頁面卡在「已探索－尚未建立索引」再協助診斷（通常是內容單薄或跟其他頁太像，不是技術錯誤）。
5. **社群貼文延伸**（依文章內容做 Threads／IG／短影音腳本）：周周先前提過但目前暫緩，非急件。
...
| `property-admin.html` | 舊網址轉址檔：`/property-admin.html`→`/admin/`（保留舊書籤相容，勿刪）。 |

---

## 9. 已知問題（非緊急，記錄用）

- 物件專區 10 筆投資店舖照片為型錄裁切、畫質僅堪用（可換原始照替換）；且無日文版 `properties-ja.html`（見第 4 節第 0 點）。
