...
<head>
<meta charset="utf-8">
<!--langredir--><script>(function(){try{var p=location.pathname.split('/').pop()||'index.html';if(p==='ja.html')return;var cn=/-cn\.html$/.test(p),L=localStorage.getItem('lang');if(L==='cn'&&!cn){location.replace(p.replace(/\.html$/,'-cn.html'));return;}if(L==='tw'&&cn){location.replace(p.replace('-cn.html','.html'));return;}}catch(e){}document.addEventListener('click',function(e){var a=e.target.closest&&e.target.closest('#langtoggle');if(!a)return;try{localStorage.setItem('lang',/-cn\.html$/.test(a.getAttribute('href')||'')?'cn':'tw');}catch(_){}} ,true);})();</script>
...
<link rel="alternate" hreflang="zh-Hans" href="${url.replace(/\.html$/,"-cn.html")}">${cover ? `
...
<body>
<div id="rp"></div>
<div class="sbar"><div class="wrap" style="display:flex;align-items:center;justify-content:space-between;padding:12px 16px"><a href="index.html" style="font-weight:800;color:var(--rose);font-size:17px">周周・日本房仲</a> <a id="langtoggle" href="#" style="font-size:13px;color:var(--mut);text-decoration:none;border:1px solid var(--line);border-radius:999px;padding:3px 10px"></a><a class="btn btn-line" href="${S.line}" target="_blank" rel="noopener">加 LINE 諮詢</a></div></div>
...
<script>(function(){var t=document.getElementById('langtoggle');if(!t)return;var p=(location.pathname.split('/').pop()||'index.html');if(/-cn\.html$/.test(p)){t.textContent='繁體中文';t.href=p.replace('-cn.html','.html');}else{t.textContent='简体中文';t.href=p.replace(/\.html$/,'-cn.html');}})();</script>
</body>
</html>`;
}
