# 每週產文主題庫（AI 自動產文用）

> 每週一的自動產文排程會從這裡挑「第一個 ⬜ 未寫」的主題寫成文章。
> 寫完請把該行改成 ✅ 並補上日期與 slug。周周可以隨時增刪、調整順序（排越前面越早寫）。
> 寫文規則一律遵守 `CLAUDE.md`：站在周周的商業利益寫、合規語氣、數據要查證附來源、繁中台灣用語、三語同步。

## ⭐ 選題優先順序（周周指示 2026-08-10）

**每週選題前，先數 `index.html` 的 `ART` 各分類目前文章數，優先補「文章數最少」的分類。**
- 六大分類：`foreign` 外國人買房／`live` 自住攻略／`invest` 投資收租／`minpaku` 民宿法規／`area` 區域介紹／`loan` 貸款稅務。
- 做法：從下面待寫清單挑「第一個 ⬜ 未寫且屬於目前最少分類」的主題；若最少分類暫時沒有合適待寫題，再往次少的分類挑。
- 目的：讓六大分類文章數平均，不要某一類（如自住）一直長、某一類（如投資、民宿）長期偏少。
- 分類現況快照（2026-08-17 重數）：`minpaku 民宿 3`、`invest 投資 4`、`area 區域 4` 最少 → 優先；`loan 5`、`foreign 7`；`live 自住 9` 最多、暫緩。

## 封面圖庫（2026-08-08 周周新增，寫新文章時優先從這裡挑、已用過的不要重複）

`pexels-francesco-siani-2150396285-31115437.jpg`（日式庭園水池）／`pexels-freestockpro-12955837.jpg`（木屋模型＋鑰匙＋文件）／`pexels-rdne-7414218.jpg`（圖表＋計算機・已用於a41）／`pexels-fotoaibe-1643383.jpg`（公寓客廳・已用於a46）／`pexels-luis-yanez-57302-206172.jpg`（洋房外觀）／`pexels-phat-tr-ng-1662052981-37785807.jpg`（現代大樓藍天）／`pexels-curtis-adams-1694007-4258277.jpg`（空房間採光）／`pexels-joanie-xie-1306424600-33752181.jpg`（獨棟外觀）
`pexels-pavel-danilyuk-7937717.jpg`（交付歐元現金＋平面圖，⚠️鈔票是歐元，日本主題慎用）
`pexels-towfiqu-barbhuiya-3440682-8732777.jpg`（西裝手持房屋模型＋計算機・已用於a45）
※ `cover-yen-handover.jpg`（交付日圓現金＋平面圖）已用於 a40。用前先 grep index.html 確認該檔名未被其他文章使用。

## 待寫主題（由上往下依序寫）

0-A. ⬜ **【周周 2026-08-23 已核准・下次優先寫】人在台灣，日本的房子誰幫我管？賃貸管理会社怎麼選、費用多少**
   分類：`invest` 投資收租。主軸放在**月租長期收租**（不是民宿、不是短租）。
   ⚠️ 周周指示：**不要寫「賃貸管理不是周周自己做的」這類自白**，用中立角度、像一般知識文章就好。
   重點：① 管理委託兩種型態（集金代行／サブリース包租）差在哪 ② 管理手数料行情
   ③ 管理公司實際做哪些事（招租、收租、催繳、修繕叫修、退租點交）
   ④ 海外屋主特有的問題（沒有日本帳戶、非居住者源泉徵收 20.42%、時差）⑤ 挑管理公司要問的問題。
   資料要查：國土交通省賃貸住宅管理業者登録制度、賃貸住宅管理業法、國稅廳非居住者源泉徵收。
   封面：`pexels-freestockpro-12955837.jpg`

0-B. ⬜ **【周周 2026-08-23 已核准・「到時候做」，順位排在 0-A 之後】不想被 180 天綁死？旅館業許可（簡易宿所）的實際門檻**
   分類：`minpaku` 民宿法規（分類文章數最少）。把 a46 只寫半節的旅館牌照展開成完整一篇。
   重點：① 旅館業許可 vs 民泊届出（365 天可營業）② 三道門檻：用途地域、構造設備基準、消防法令適合通知書
   ③ 簡易宿所的面積與設備基準 ④ **買帶牌照的建物 ≠ 自動繼承牌照**（許可發給經營者；2023/12 修法後事業讓渡可申請承繼，
   單純買建物原則仍須重新申請）⑤ 舊條例適用物件為什麼值錢。
   ⚠️ 合規：全篇寫明「能否取得依所在自治體條例與保健所判斷，請由行政書士與保健所確認」，不暗示一定拿得到。
   封面：`pexels-phat-tr-ng-1662052981-37785807.jpg`

1. ⬜ **【周周 2026-08-23 指定】地震來了怎麼逃？日本人從小做的防災訓練，與住在日本要準備的事**
   分類：`life` 生活資訊。切入角度：日本是地震頻繁的國家，小學就開始做避難訓練（避難訓練・防災頭巾・引き渡し訓練），
   所以日本人面對地震的反應跟我們不一樣。寫給要來日本住／已經買房的台灣人：地震當下怎麼做、
   家裡要準備什麼（家具固定、防災包、備蓄）、避難場所怎麼查、緊急地震速報怎麼開、
   在留卡與護照怎麼放。結尾自然接到「買房時我會幫你看耐震標準與 hazard map」，
   並連到既有的 a15 耐震／a25 hazard map／a26 防震三工法。
   資料來源要查：內閣府防災情報、氣象廳緊急地震速報、東京都「東京防災」、各區避難場所公告。

2. ⬜ 火災保險・地震保險怎麼保？日本買房後的保險入門
3. ⬜ 塔式住宅（タワーマンション）優缺點總整理：買塔樓前必看
4. ⬜ 收租族必看：管理公司（賃貸管理会社）怎麼選？委託管理做什麼、行情多少
5. ⬜ 空室對策：房子租不出去怎麼辦？降租之外的 5 個方法
6. ⬜ 中古屋翻新（リフォーム／リノベーション）費用行情與流程
7. ⬜ 事故物件（凶宅）怎麼查？大島てる與告知義務的規定
8. ⬜ 定期借地權物件是什麼？便宜背後的代價，能不能買
9. ⬜ 建売住宅 vs 注文住宅：日本新築一戶建的兩種買法
10. ✅ 2026-08-16 japan-property-company-vs-individual 法人名義 vs 個人名義買房（a43・invest）
11. ⬜ 管理組合是什麼？買公寓後的理事會、總會與你的權利義務
12. ⬜ 車站距離對房價與轉手性的影響：徒步幾分是分水嶺？
13. ⬜ 日本房貸提前還款（繰上返済）划算嗎？該還本金還是縮年限
14. ⬜ 買房收租後的確定申告（報稅）入門：什麼要報、找誰報
15. ✅ 2026-08-17 minpaku-monthly-rental-comparison 民宿 vs 月租公寓（マンスリー）vs 一般出租：三種收益模式比較（a46・minpaku）
16. ⬜ 建物檢查（ホームインスペクション）是什麼？中古屋要不要驗屋
17. ⬜ 外國人持有日本房產的繼承（相続）問題入門（結尾提醒：依司法書士／稅理士確認）
18. ⬜ 出租時的禮金、敷金、廣告費（AD）行情：房東要付哪些成本
19. ⬜ 高齡公寓的大規模修繕：買 30 年以上中古屋前要看的修繕履歷
20. ⬜ 日本房屋的斷熱・省能源等級入門：2025 年新基準後怎麼挑
21. ⬜ 東京 vs 大阪 vs 福岡：三大都市投資買房比較
22. ⬜ 日圓匯率與買房時機：匯率怎麼影響台灣買家的實際成本
23. ⬜ 東京的學區宅入門：文京區等名門學區為什麼保值
24. ⬜ 從看房到交屋要多久？日本買房時間軸全解析

## 已完成

（完成的主題移到這裡，格式：✅ 2026-08-15 `slug-name` 主題）
✅ 2026-08-10 `how-to-transfer-money-japan-property` 日本買房的錢怎麼匯？海外匯款流程、時間點與匯率
✅ 2026-08-17 `japan-rental-yield-gross-vs-net` 表面利回り vs 実質利回り：外國人買收租房最常誤判的 5 件事（a45・invest）
✅ 2026-08-17 `minpaku-monthly-rental-comparison` 民宿 vs 月租公寓 vs 一般出租：三種收益模式比較（a46・minpaku）
